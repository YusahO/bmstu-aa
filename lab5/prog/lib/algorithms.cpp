...
static std::tuple<int, std::vector<std::string>> read_descr_file(const std::string &descr_file)
{
    std::ifstream f{descr_file};
    if (!f.is_open())
        throw std::runtime_error("Unable to open file " + descr_file);
...
{
    auto [id, filenames] = read_descr_file(descr_file);
    ...
    for (const auto &filename : filenames)
    {
        ...
        for (int track = 0; track < midifile.getTrackCount(); track++)
        {
            for (int event = 0; event < midifile[track].size(); event++)
            {
                ...
                {
                    temp_notes.push_back(midifile[track][event][1]);
                }
            }
        }
...
{
    srand(time(NULL));
    membership_t mshp(d, std::vector<double>(k, 0.0));
    point_vec_t cc(k, std::vector<double>(2));
    for (int i = 0; i < k; ++i)
    {
        cc[i][0] = static_cast<double>(rand()) / RAND_MAX * 5.0;
        cc[i][1] = static_cast<double>(rand()) / RAND_MAX * 3.5;
    }
    return { mshp, cc };
}
